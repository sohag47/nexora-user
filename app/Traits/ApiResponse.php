...
trait ApiResponse
{
    ...
    protected function success($data = null, string $message = '', int $status = Response::HTTP_OK)
    {
        $response_message = empty($message) ? ResponseStatusEnums::SUCCESS->value : $message;
    ...
    protected function updated(string $message = '')
    {
        // return $this->success($data, empty($message) ? ResponseStatusEnums::UPDATED->value : $message, Response::HTTP_OK);
        $response_message = empty($message) ? ResponseStatusEnums::UPDATED->value : $message;

        return response()->json($response_message, empty($message) ? Response::HTTP_NO_CONTENT : Response::HTTP_OK);
    }

    protected function deleted(string $message = '')
    {
        $response_message = empty($message) ? ResponseStatusEnums::DELETED->value : $message;
